...
codeunit 13636 "OIOUBL-Export Sales Invoice"
{
    ...
    var
        ...
        DSMPermMgt: Codeunit "DSM Permission Management";
        RecordRef: RecordRef;
    ...
    begin
        ...
        //DSMJMA#8 ->
        DSMPermMgt.CheckPermission('C13636');
        //DSMJMA#8 <-
    ...
    var
        ...
        DSMDKSetup: Record "DSM DK Setup";
    ...
    var
        ...
        Text6038950: Label '-SAF-';
        Text6038951: Label '-MAF-';
        PleaseUsetheStdInvoicePath: Label 'Please save the file under %1.';
        UsetheIOULBClientPath: Label 'Please save the file to a location from which it can be retrieved by the OIOUBL function.';
    ...
    begin
        ...
        SalesSetup.TESTFIELD("X-No.");                                             //DSMDK#2006
        SalesInvHeader2.GET(SalesInvoiceHeader."No.");                             //DSMDK#2006
        IF SalesInvHeader2."Service Item Document" THEN                            //DSMDK#2006
            InvNo := SalesSetup."X-No." + Text6038951 + SalesInvHeader2."No."        //DSMDK#2006
        ELSE                                                                       //DSMDK#2006
            InvNo := SalesSetup."X-No." + Text6038950 + SalesInvHeader2."No.";       //DSMDK#2006

        IF RBMgt.IsNASClient() OR (NOT ClientFileAvailable(FromFile)) THEN //DSMJMA#5007
            RBMgt.CopyServerFile(FromFile, STRSUBSTNO('%1\%2.xml', SalesSetup."OIOUBL-Invoice Path", InvNo), TRUE) //DSMJMA#5001
        ELSE //DSMJMA#5001
            if RBMgt.CanRunDotNetOnClient() and not PermissionManager.SoftwareAsAService() then begin
                SalesSetup.VerifyAndSetOIOUBLSetupPath(DocumentType::Invoice);
                ToFile := RBMgt.DownloadTempFile(FromFile);
                RBMgt.CopyClientFile(ToFile, STRSUBSTNO('%1\%2.xml', SalesSetup."OIOUBL-Invoice Path", InvNo), TRUE);   //DSMJMA#5001
            END ELSE BEGIN
                ...
                FileName := STRSUBSTNO('%1.xml', InvNo);                      //DSMDK#2006
        ...
        //DSMDK#2016 ->
        IF SalesInvHeader2."No. Printed" = 0 THEN
            SalesInvHeader2."No. Printed" := 1;
        //DSMDK2016 <-
        SalesInvHeader2.MODIFY();
    end;

    local procedure ClientFileAvailable(FilePath: Text): Boolean
    var
        FileManagement: Codeunit "File Management";
        ClientFileHelper: DotNet SystemIOFile;
    begin
    ...
    var
        ...
        BetalingsType: Text[50];
        "Fik/Giro-No.": Code[20];
        DealerServMgt: Codeunit DealerServiceManagement;

    begin
        CODEUNIT.RUN(CODEUNIT::"OIOUBL-Check Sales Invoice", SalesInvoiceHeader);
        ReadGLSetup();
        ReadCompanyInfo();
        IF NOT DSMDKSetup.FINDFIRST() THEN //DSMJMA#5008
            DSMDKSetup.INIT();               //DSMJMA#5008
        ...
        //DSMJMA#3001 ->
        //DSMJMA#1 ->
        DealerServMgt.SalesHeaderPaymentMeans(SalesInvoiceHeader, Kortart, "Betalings-ID", BetalingsType, "Fik/Giro-No.");
        IF Kortart <> '' THEN
            OIOUBLXMLGenerator.InsertPaymentMeansDSM(XMLCurrNode, SalesInvoiceHeader."Due Date", Kortart, "Betalings-ID", BetalingsType, "Fik/Giro-No.")
        ELSE
