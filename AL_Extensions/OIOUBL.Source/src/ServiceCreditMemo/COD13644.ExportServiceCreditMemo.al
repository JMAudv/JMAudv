...
codeunit 13644 "OIOUBL-Export Service Cr.Memo"
{
    ...
    var
        ...
        DecimalBuffer: Record "Decimal Buffer";
        ServCrMemoLine2: Record "Service Cr.Memo Line";
        ServCrMemoLineTemp: Record "Service Cr.Memo Line" temporary;
        TempPostServItemLine: Record "Posted Service Item Line Cr." temporary;
        ...
        GLSetupRead: Boolean;
        TaxTotalWritten: Boolean;
        TempLineNo: Integer;
        DiscountVATCounter: Integer;
        FixedPriceGrList: DotNet FixedPriceGrList;
        UsedLineNo: dotnet UsedLineNo;
    ...
    begin
        //DSMJMA ->
        TempLineNo := 999999;    //DSMJMA#4379
        TaxTotalWritten := FALSE; //DSMJMA#3003
        DealerServMgtSetup.GET();
        IF NOT DealerServMgtSetup."Use OIOXML" THEN
            EXIT;
        DSMPermMgt.CheckPermission('C13616');
        //DSMJMA <-
    ...
    var
        ...
        GlobalServiceCrMemoHeader: Record "Service Cr.Memo Header";
        RBMgt: Codeunit "File Management";
    ...
    begin
        ...
        ////DSMJMA#5009 ->
        IF NOT DSMDKSetup.FINDFIRST() THEN
            DSMDKSetup.INIT();
        ////DSMJMA#5009 <-
        ...
        //DSMJMA#5011 ->
        IF (ServCrMemoLine."DSM Serv. Price Adjmt. Gr.Code" <> '') AND (ServCrMemoLine."Price Adjmt. Status" = ServCrMemoLine."Price Adjmt. Status"::Adjusted) THEN
            ServCrMemoLine."Line Discount Amount" := 0;

        GlobalServiceCrMemoHeader := ServiceCrMemoHeader;
        CLEAR(FixedPriceGrList);
        FixedPriceGrList := FixedPriceGrList.List();
        CLEAR(UsedLineNo);
        UsedLineNo := UsedLineNo.List();
        //DSMJMA#5011 <-
        ...
        with ServiceCrMemoHeader do begin
            ...
            //DSMJMA#2006 ->
            DecimalBuffer.DELETEALL();
            DiscountVATCounter := 1;
            //DSMJMA#2006 <-
            ...
            if ServiceCrMemoLine2.FINDSET() then
                repeat
                    ...
                    //DSMJMA#2006 ->
                    IF DecimalBuffer.GET(ServCrMemoLine2."VAT %") THEN BEGIN
                        DecimalBuffer."Amount 1" := DecimalBuffer."Amount 1" + ServCrMemoLine2."Inv. Discount Amount" +
                                                    ServCrMemoLine2."Line Discount Amount";
                        DecimalBuffer.MODIFY();
                    END ELSE
                        IF (ServCrMemoLine2."Inv. Discount Amount" + ServCrMemoLine2."Line Discount Amount") > 0 THEN BEGIN
                            DecimalBuffer.DecimalKey := ServCrMemoLine2."VAT %";
                            DecimalBuffer."Amount 1" := ServCrMemoLine2."Inv. Discount Amount" + ServCrMemoLine2."Line Discount Amount";
                            DecimalBuffer."Integer 1" := DiscountVATCounter;
                            DecimalBuffer."Integer 2" := ServCrMemoLine2."VAT Calculation Type";
                            DecimalBuffer.INSERT();
                            DiscountVATCounter += 1;
                        END;
                    //DSMJMA#2006 <-
        ...
        //DSMJMA#4379 ->
        ServCrMemoLineTemp.DELETEALL();
        TempPostServItemLine.DELETEALL();
        repeat
